...
const Cards = ({ home, setInputDiv, data, setUpdatedData }) => {
  ...
  const handleCompleteTask = async (id) => {
    try {
      await axios.put(
        `${API_BASE_URL}/update-complete-task/${id}`,
        {},
        { headers }
      );
    } catch (error) {
  ...
  const handleImportantTask = async (id) => {
    try {
      await axios.put(
        `${API_BASE_URL}/update-imp-task/${id}`,
        {},
        { headers }
      );
    } catch (error) {
  ...
  const deleteTask = async (id) => {
    try {
      await axios.delete(`${API_BASE_URL}/delete-task/${id}`, {
        headers,
      });
    } catch (error) {
      console.log(error);
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 p-4">
      {data &&
        data.map((items, i) => (
          <div
            key={i}
            className="flex flex-col justify-between border border-black-700 bg-gradient-to-r from-yellow-50 to-red-50 rounded-lg p-6 shadow-lg hover:scale-105 transition-transform duration-300"
          >
            <div>
              <h3 className="text-xl font-semibold text-black truncate">
                {items.title}
              </h3>
              <p className="text-black my-2 text-sm">{items.desc}</p>
            </div>
            <div className="mt-4 flex flex-wrap justify-between items-center">
              <button
                ...
                } px-4 py-2 rounded text-black text-sm transition duration-300 hover:bg-opacity-80 w-full sm:w-auto`}
                onClick={() => handleCompleteTask(items._id)}
              >
                {items.complete === true ? "Completed" : "Incomplete"}
              </button>
              <div className="text-black p-2 text-xl flex justify-around mt-2 sm:mt-0 w-full sm:w-auto">
